      ...
      <div className="mx-auto w-full max-w-sm rounded-3xl bg-[#0b1f1e] p-3 shadow-lg">
        ...
        <div className="flex items-center gap-3 px-2 pb-3 pt-1">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-emerald-300/80">{t("coachingUpdates")}</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl rounded-tl-md bg-[#dcf8c6] p-4 text-[#0b2e16] shadow-sm">
          ...
          <div className="my-3 rounded-xl bg-white/70 p-3">
            ...
            <p className="font-display text-2xl font-bold text-[#0b2e16]">
              ...
              <span className="ml-2 align-middle text-sm font-medium text-[#0b2e16]/70">
                ({t("accuracy", { pct: report.accuracyPct })})
              </span>
            </p>
          </div>

          <p className="text-sm leading-relaxed">
            💪 <span className="font-semibold">{t("strongArea")}</span>{" "}
            {report.strongestSubject
              ? t("strongAreaBody", { subject: report.strongestSubject, firstName })
              : "—"}
          </p>

          <p className="mt-2 text-sm leading-relaxed">
            📌 <span className="font-semibold">{t("needsWork")}</span>
          </p>
          ...
          <div className="mt-3 rounded-xl bg-[#0b2e16]/[0.06] p-3">
            <p className="text-sm leading-relaxed">
              🎯 <span className="font-semibold">{t("thisWeeksFocus")}</span>{" "}
              {focus ? focus.advice : t("defaultFocus")}
            </p>
            {focus && focusGroup && (
              <p className="mt-1.5 text-xs font-medium text-[#0b2e16]/55">
                {t("basedOn", { count: focusGroup.items.length, confidence: focusGroup.avgConfidence })}
              </p>
            )}
          </div>

          <p className="mt-3 text-sm leading-relaxed">
            {t("noWorries", { firstName })}
          </p>
